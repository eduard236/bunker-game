...
class SecretMissionSystem {
        ...
        holders.forEach(playerIndex => {
            ...
            if (missionType.type === "HEALER") {
                ...
                for (let i = 0; i < players.length; i++) {
                    if (i !== playerIndex && players[i].health !== "Идеальное здоровье") {
                        candidates.push(i);
                    }
                }
                ...
                } while (targetPlayerIndex === playerIndex);
            }
    ...
    getMissionForPlayer(playerIndex) { 
        return this.missions.find(m => m.playerIndex === playerIndex); 
    }

    hasMission(playerIndex) { 
        return this.missions.some(m => m.playerIndex === playerIndex); 
    }
}
...
class BunkerGame {
    ...
    init() { 
        ...
        this.populateBackstories();
    }

    populateBackstories() {
        const select = document.getElementById('backstorySelect');
        if (!select) return;
        
        gameData.backstories.forEach((b, i) => {
    ...
    generateGame(playerCount, backstoryIndex) {
        ...
        const pools = {
            profession: shuffle([...gameData.professions]), 
            health: shuffle([...gameData.health]),
            phobia: shuffle([...gameData.phobias]), 
            hobby: shuffle([...gameData.hobbies]),
            trait: shuffle([...gameData.traits]), 
            fact: shuffle([...gameData.facts]),
    ...
    showMissionsScreen() {
        ...
        if (this.currentMissionStep >= this.missionSystem.missions.length) { 
            this.showGameScreen(); 
            return; 
        }
    ...
    bindGameScreen(root) {
        ...
        root.querySelectorAll('.exile-btn').forEach(b => b.addEventListener('click', () => {
            ...
            const card = b.closest('.player-card');
            if (this.exiled.has(idx)) { 
                this.exiled.delete(idx); 
                card.classList.remove('exiled'); 
                b.textContent = '🚪 Изгнать'; 
            } else { 
                this.exiled.add(idx); 
                card.classList.add('exiled'); 
                b.textContent = '↩ Вернуть в бункер'; 
            }
            this.updateStatus();
        }));
    ...
    showPersonalMission(playerIndex) {
        ...
        modal.innerHTML = `
            <div class="modal-content mission-content-box">
                ...
                <div class="mission-warning-orange">
                    <p>⚠️ НИКОМУ не показывайте эту миссию! Держите её в строгом секрете!</p>
                </div>
...
function login() {
    ...
    successDiv.textContent = 'Вход выполнен! (демо режим)';
...
function register() {
    ...
    successDiv.textContent = 'Регистрация успешна! (демо режим)';
    successDiv.classList.add('show');
    errorDiv.classList.remove('show');
}
...
function forgotPassword() {
    ...
    successDiv.textContent = 'Ссылка отправлена на email! (демо режим)';
    successDiv.classList.add('show');
    errorDiv.classList.remove('show');
}